...
// Theme initialization and toggle for root static demo
function applyThemeRoot(t){
  try{ document.documentElement.className = t; }catch(e){}
  const btn = document.getElementById('themeToggle');
  if(btn) btn.textContent = (t === 'dark') ? '🌙' : '☀️';
}
const storedThemeRoot = (function(){ try{ return localStorage.getItem('dashboard_theme'); }catch(e){return null} })();
const initialThemeRoot = storedThemeRoot || (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light');
applyThemeRoot(initialThemeRoot);
const themeBtnRoot = document.getElementById('themeToggle');
if(themeBtnRoot){ themeBtnRoot.addEventListener('click', ()=>{ const next = document.documentElement.className === 'dark' ? 'light' : 'dark'; applyThemeRoot(next); try{ localStorage.setItem('dashboard_theme', next); }catch(e){} }); }

// Navigation: smooth-scroll to section and update active nav
...
// Analytics grid duplicates cards but smaller (simple)
...
// Check alerts against metrics and notify (subtle)
function checkAlerts(){
  alerts.forEach(a=>{
    ...
    if((a.condition==='above' && val > a.threshold) || (a.condition==='below' && val < a.threshold)){
      // trigger visual flash on matching card
...
el('toggleStats').addEventListener('click', ()=>{
  ...
  // simple behavior: toggle a small class that can be used later
  document.body.classList.toggle('show-stats', !showing);
});

// initial population of cards once DOM is ready
...
// small accessibility: keyboard navigation for nav
